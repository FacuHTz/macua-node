...
(function () {
  ...
  function cardTpl(v) {
    ...
    const title = `${v.brand} ${v.model} ${v.year || ""}`.trim();
    const url = `/vehiculos/${v.id}`;
    return `
      <article class="card">
        <a class="thumb" href="${url}" style="background-image:url('${thumb}')" aria-label="${title}"></a>
        <div class="body">
          <div class="badges">
            ${
              v.zero_km
        ...
        <div class="foot">
          <a class="btn btn-primary" href="${url}">Ver detalle</a>
          <a class="btn btn-light" href="https://wa.me/5493430000000?text=${encodeURIComponent(
            `Hola! Me interesa el ${title} (${fmtARS(v.price)}).`
